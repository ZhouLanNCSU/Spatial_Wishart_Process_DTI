function[Bookkeeping_Current_mapObj]=MCMC_beta_diag_ModifiedModel_NNGP(it,Bookkeeping_Input_mapObj,Bookkeeping_Data_mapObj, Bookkeeping_Priors_mapObj, Bookkeeping_Current_mapObj)
     ...
     for dim_index=1:ndim
        for pred_index=1:npred
          candidate=normrnd(beta11(dim_index,pred_index),0.08);
          beta11_can=beta11;
          ...
          can11=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t11_partial_can.^2);
          old11=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t11_partial.^2);
...
count/(ndim*npred)
     ...
     for dim_index=1:ndim
        for pred_index=1:npred
          candidate=normrnd(beta22(dim_index,pred_index),0.08);
          beta22_can=beta22;
          ...
          can22=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t22_partial_can.^2);
          old22=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t22_partial.^2);
...
count/(ndim*npred)
     ...
     for dim_index=1:ndim
        for pred_index=1:npred
          candidate=normrnd(beta33(dim_index,pred_index),0.08);
          beta33_can=beta33;
          ...
          can33=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t33_partial_can.^2);
          old33=-0.5*sum((1/R_c_F_all(dim_index)-1)*u_t33_partial.^2);
