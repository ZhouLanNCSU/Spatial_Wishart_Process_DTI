function [Bookkeeping_Current_mapObj]=Log_Gamma_V1_MCMC_R_dof_ModifiedModel(it,Bookkeeping_Input_mapObj,Bookkeeping_Data_mapObj, Bookkeeping_Priors_mapObj, Bookkeeping_Current_mapObj)
     ...
     t_central_all_can(1)={log_t11_central_can};
     t_central_all_can(2)={log_t22_central_can};
     t_central_all_can(3)={log_t33_central_can};
     ...
     for dindex=1:3
        ...
        cankk(dindex)=sum(arrayfun(@(s) likelihood_dof( s,t_central_can, R_Xi_Choleksy_Lower_inv,R_Xi_det,sigma_Xi_all_can(dindex),ndim),[1:nsub]));
        oldkk(dindex)=sum(arrayfun(@(s) likelihood_dof( s,t_central, R_Xi_Choleksy_Lower_inv,R_Xi_det,sigma_Xi_all(dindex),ndim),[1:nsub]));         
     end
     ...
     if Accept==1
         ...
         Current_others_mapObj('log_t11_central')=log_t11_central_can;
         Current_others_mapObj('log_t22_central')=log_t22_central_can;
         Current_others_mapObj('log_t33_central')=log_t33_central_can;
